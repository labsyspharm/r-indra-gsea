...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "indra.gsea", "tests")); system.time(test_check("indra.gsea", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran indra.gsea_$(VERSION).tar.gz
	rm -rf indra.gsea.Rcheck/
